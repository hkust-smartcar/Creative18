...
//		test->writer.WriteString(c);
	...
	if (pWheelbase) {
		pWheelbase->encoder0.Update();
		if ((pWheelbase->encoder0.GetCount() < 400) && (pWheelbase->encoder0.GetCount() > -400)) {
			pWheelbase->encoder_counts[0] += pWheelbase->encoder0.GetCount();
		}
