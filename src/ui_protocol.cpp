...
uint8_t UiProtocol::RequestSetMotorById(uint8_t id, int16_t speed) {
	...
	return m_bt.QueuePackage(
			{ Bluetooth::PkgType::kRequestSetMotorById, 0, { } });
}
...
uint8_t UiProtocol::ResponseEncoderById(uint8_t id, int32_t count) {
	...
	return m_bt.QueuePackage( { Bluetooth::PkgType::kResponseEncoderById, 0,
			data });
}

uint8_t UiProtocol::ResponseEncoders(int32_t count0, int32_t count1,
		int32_t count2) {
	...
	return m_bt.QueuePackage(
			{ Bluetooth::PkgType::kResponseEncoders, 0, data });
}

void UiProtocol::ResponseEncoderByIdImmediate(uint8_t id, int32_t count) {
	vector<Byte> data(5, 0);
	memcpy(&*data.begin(), &id, 1);
	memcpy(&*data.begin() + 1, &count, 4);
	return m_bt.SendPackageImmediate( { Bluetooth::PkgType::kResponseEncoderById, 0,
			data });
}

void UiProtocol::ResponseEncodersImmediate(int32_t count0, int32_t count1,
		int32_t count2) {
	vector<Byte> data(12, 0);
	memcpy(&*data.begin(), &count0, 4);
	memcpy(&*data.begin() + 4, &count1, 4);
	memcpy(&*data.begin() + 8, &count2, 4);
	return m_bt.SendPackageImmediate(
			{ Bluetooth::PkgType::kResponseEncoders, 0, data });
}
...
void UiProtocol::RequestSetMotorByIdHandler(const Bluetooth::Package& pkg) {
	...
	if (pWheelbase) {
	//	pWheelbase->motor_target_velocity[motor_id]=speed;
		//pWheelbase->MotorSetPower(motor_id, speed);
		switch (motor_id) {
		case 0:
			pWheelbase->target_x_velocity = speed;
			break;
		case 1:
			pWheelbase->target_y_velocity = speed;
			break;
		case 2:
			pWheelbase->target_w_velocity = speed;
			break;
		}
		pWheelbase->set_motor_target_velocity();
...
void UiProtocol::RequestEncoderByIdHandler(const Bluetooth::Package& pkg) {
	...
	if (pWheelbase) {
		ResponseEncoderById(encoder_id,
				pWheelbase->EncoderGetCount(encoder_id));
	}
}

void UiProtocol::RequestEncodersHandler(const Bluetooth::Package& pkg) {
	if (pWheelbase) {
		ResponseEncoders(pWheelbase->EncoderGetCount(0),
				pWheelbase->EncoderGetCount(1), pWheelbase->EncoderGetCount(2));
	}
}
